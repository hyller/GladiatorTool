...
macro CmtReformat3()
{
	...
	rec = ___tsGetStartColumn(hbuf, ichFirst, sel.lnFirst);
	...
	if (rec.colComment >= 0)
	{
		...
		szMargin = ___tsAddWhiteToColumn(szMargin, 0, rec.colComment, tabSize);
		szMargin = cat(szMargin, "///");
	}

	szMargin = ___tsAddWhiteToColumn(szMargin, colComment, rec.colMargin, tabSize);
...
//		msg("$0-" # iLine # ":" # szRemainder);
		rec = ___tsGetNextCommentString(hbuf, ichFirst, szRemainder, iLine, sel.lnLast, colWrap);
		ichFirst = 0;
...
//		msg("Leftovers @szRemainder@");
		...
		while (ich < len)
		{
			...
			fIsWhitespace = ___tsIsWhitespaceChar(szLine[ich]);
			fIsWhitespace1 = 1;

			if (ich + 1 < len)
			{
				fIsWhitespace1 = ___tsIsWhitespaceChar(szLine[ich + 1]);
			}
...
//			msg("$2:" # strmid(szLine, 0, ichPrevCharToWhite + 1));
			...
			while (ich < len)
			{
				if (!___tsIsWhitespaceChar(szLine[ich]))
				{
...
macro ___tsAddWhiteToColumn(sz, col0, col, tabSize)
{
	szSpaces = "                                                                                                    ";
...
macro ___tsGetStartColumn(hbuf, ichBegin, ln)
{
	...
	while (ich < len)
	{
		...
		if (colComment < 0)
		{
			if (ich + 1 < len)
			{
				if (szLine[ich] == "/" && szLine[ich+1] == "/" && szLine[ich+2] == "/")
				{
...
macro ___tsGetNextCommentString(hbuf, ichSkip, szRemainder, ln, lnLast, colWrap)
{
	rec = "";
...
//	msg(ichSkip);
	while (1)
	{
		...
		if (ichSkip == 0)
			ichFirst = ___tsSkipPastCommentAndWhitespace(szLine, len);
		ichSkip = 0;
		...
		{
			if (!___tsIsWhitespaceChar(szLine[ichLast]))
			{
				break;
			}
			ichLast = ichLast - 1;
...
macro ___tsSkipPastCommentAndWhitespace(szLine, len)
{
	ichFirst = 0;
	...
	while (ichFirst < len)
	{
		if (ichFirst + 1 < len)
		{
			if (szLine[ichFirst] == "/" && szLine[ichFirst+1] == "/" && szLine[ichFirst+2] == "/")
			{
				ichFirst = ichFirst + 3;
				break;
			}
	...
	while (ichFirst < len)
	{
		if (!___tsIsWhitespaceChar(szLine[ichFirst]))
		{
			break;
		}
		ichFirst = ichFirst + 1;
...
macro ___tsIsWhitespaceChar(ch)
{
  if(ch == " " || ch == "\t")
    return 1
  else
