...
#  endif
#else
#include <iostream> // LGR
#endif

#include <cppunit/TestCase.h> // LGR
#include <cppunit/TestSuite.h> // LGR
...
{ 
  ExclusiveZone zone( m_syncObject ); 
  TestCase *tc = CPPUNIT_CONST_CAST(TestCase *, test); // LGR
#if defined (CPPUNIT_WITHOUT_CPP_STREAM)
#  if !defined (CPPUNIT_WITHOUT_C_OUTPUT)
  printf("\n\t");
  printf(tc->getName().c_str());
  printf(" : ");
#  endif
#else
  std::cerr  <<  std::endl << "\t" << tc->getName() << " : " ; // LGR
#endif
  for ( TestListeners::iterator it = m_listeners.begin();
        it != m_listeners.end(); 
        ++it )
...
{
  ExclusiveZone zone( m_syncObject ); 
  TestSuite *ts = CPPUNIT_CONST_CAST(TestSuite *, test); // LGR
#if defined (CPPUNIT_WITHOUT_CPP_STREAM)
#  if !defined (CPPUNIT_WITHOUT_C_OUTPUT)
  printf("\n");
  printf(ts->getName().c_str());
  printf(" : begin");
#  endif
#else
  std::cerr  <<  std::endl << ts->getName() << " : begin" ; // LGR
#endif
  for ( TestListeners::iterator it = m_listeners.begin();
        it != m_listeners.end(); 
        ++it )
...
#  endif
#else
  std::cerr  << std::endl << ts ->getName() << " : end " <<  std::endl ; // LGR
#endif
}
